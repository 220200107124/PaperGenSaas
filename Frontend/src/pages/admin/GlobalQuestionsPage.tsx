...
const GlobalQuestionsPage: React.FC = () => {
  ...
  const [newQuestions, setNewQuestions] = useState<BulkQuestionState[]>([
    createEmptyQuestion(),
  ]);
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm("Are you sure you want to delete this global question?")
    )
      return;
    try {
  ...
  const handleEdit = (q: Question) => {
    ...
    setModalData({
      ...
      options: q.options
        ...
        : ["", "", "", ""],
      answer: q.answer?.correctOption
        ? ["a", "b", "c", "d"].indexOf(q.answer.correctOption).toString()
        : q.answer?.text || "",
  ...
  const handleSave = async () => {
    try {
      ...
      if (modalData.questionType === QuestionType.MCQ) {
        qOptions = {
          a: modalData.options[0],
          b: modalData.options[1],
          c: modalData.options[2],
          d: modalData.options[3],
        };
        qAnswer = {
          correctOption: ["a", "b", "c", "d"][parseInt(modalData.answer) || 0],
        };
      } else {
      ...
      if (editingQuestion) {
        await axios.patch(
          `${import.meta.env.VITE_API_URL}/questions/${editingQuestion.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
  ...
  const fetchStandards = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/standards?limit=100`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchSubjects = async (stdId: string) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/subjects?limit=100&standardId=${stdId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchChapters = async (subId: string) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/chapters?limit=100&subjectId=${subId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const saveQuestion = async (index: number) => {
    ...
    try {
      ...
      if (q.questionType === QuestionType.MCQ) {
        ...
        qAnswer = {
          correctOption: ["a", "b", "c", "d"][parseInt(q.answer) || 0],
        };
      } else {
  ...
  const handleAIUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/questions/extract-pdf`,
        formData,
        {
          headers: {
      ...
      const extracted = response.data.data.map((q: any) => ({
        ...
        answer:
          q.questionType === QuestionType.MCQ
            ? ["a", "b", "c", "d"].indexOf(q.answer).toString()
            : q.answer,
        status: "pending",
      }));
      ...
      toast.error(
        error.response?.data?.message || "Failed to extract questions",
      );
    } finally {
  ...
  const handlePublish = async () => {
    ...
    try {
      for (let i = 0; i < newQuestions.length; i++) {
        ...
        if (
          !newQuestions[i].questionText ||
          newQuestions[i].questionText === "<p><br></p>"
        )
          continue;
      ...
      toast.error(
        error.response?.data?.message || "Failed to publish all questions",
      );
    } finally {
      setPublishing(false);
    }
        ...
        <div className="flex flex-col gap-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <div
              ...
              className="text-lg font-bold text-gray-900 leading-relaxed font-gujarati max-w-2xl"
            />
          </div>
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-gray-400 font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-2">
            Gujarat Board <ChevronRight className="w-3 h-3" />{" "}
            <span className="text-brand-blue">Standard Syllabus</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="px-8 py-4 bg-brand-blue text-white rounded-2xl font-black text-xs sm:text-sm shadow-xl shadow-brand-blue/20 flex items-center gap-3 active:scale-95 transition-all hover:bg-blue-800"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 md:gap-6 bg-white p-4 sm:p-6 lg:p-8 rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/20 items-end">
        ...
        <div className="lg:col-span-3 space-y-2">
          ...
          <CustomSelect
            ...
            value={filterChapter}
            onChange={(val: string) => setFilterChapter(val)}
        ...
        footer={
          <>
            <button
              ...
              className="px-8 py-3 bg-white border border-gray-100 rounded-2xl text-sm font-black text-gray-400 hover:bg-gray-50 transition-all"
            >
              Cancel
            </button>
            <button
            ...
            >
              <PlusCircle
                className={clsx("w-5 h-5", publishing && "animate-spin")}
              />
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50 p-6 rounded-3xl">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full bg-white border-0 px-4 py-3 rounded-xl font-bold text-xs focus:ring-4 focus:ring-brand-blue/5 outline-none"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-1.5">
              ...
              <select
                value={newQuestions[activeQuestionIndex]?.questionType || ""}
                onChange={(e) => {
                  const updated = [...newQuestions];
                  updated[activeQuestionIndex].questionType = e.target
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={newQuestions[activeQuestionIndex]?.difficulty || ""}
                onChange={(e) => {
                  const updated = [...newQuestions];
                  updated[activeQuestionIndex].difficulty = e.target
                    .value as Difficulty;
                  setNewQuestions(updated);
                }}
                className="w-full bg-gray-50 border-0 px-4 py-3 rounded-xl font-bold text-xs outline-none"
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="number"
                value={newQuestions[activeQuestionIndex]?.marks || 1}
                onChange={(e) => {
                  const updated = [...newQuestions];
                  updated[activeQuestionIndex].marks =
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-[400px]">
            <div className="lg:col-span-4 flex flex-col gap-4 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
                ...
                <div
                  ...
                  className={clsx(
                    ...
                    activeQuestionIndex === idx
                      ...
                      : "bg-white border-gray-100 text-gray-600 hover:border-brand-blue/30",
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col overflow-hidden flex-1">
                      <p className="text-xs font-bold truncate font-gujarati">
                        {q.questionText?.replace(/<[^>]*>/g, "") ||
                          "Empty Content"}
                      </p>
                      <p className="text-[9px] font-bold opacity-70 uppercase tracking-wider">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {q.status !== "saved" && q.status !== "saving" && (
                        <button
                          ...
                          disabled={
                            !modalStandard || !modalSubject || !modalChapter
                          }
              ...
              <button
                onClick={() => {
                  setNewQuestions([...newQuestions, createEmptyQuestion()]);
                  setActiveQuestionIndex(newQuestions.length);
                }}
            ...
            <div className="lg:col-span-8 space-y-4">
              ...
              {newQuestions[activeQuestionIndex]?.questionType ===
              QuestionType.MCQ ? (
                    ...
                    {["A", "B", "C", "D"].map((opt, i) => (
                      <div key={i} className="flex gap-3 items-center">
                        ...
                        <input
                          ...
                          value={
                            newQuestions[activeQuestionIndex]?.options[i] || ""
                          }
                          onChange={(e) => {
                            const updated = [...newQuestions];
                            updated[activeQuestionIndex].options[i] =
                              e.target.value;
                            setNewQuestions(updated);
                          }}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button
              onClick={handleSave}
              className="px-12 py-3 bg-brand-orange text-white rounded-2xl font-black text-sm shadow-xl shadow-brand-orange/20 hover:bg-orange-600 transition-all"
            >
            ...
            <div className="space-y-2">
              ...
              <div className="bg-white border-2 border-gray-50 rounded-[2rem] overflow-hidden">
                <ReactQuill
                  ...
                  onChange={(v) =>
                    setModalData((prev) => ({ ...prev, questionText: v }))
                  }
          ...
          <div className="space-y-6 bg-gray-100/30 p-8 rounded-[2.5rem] border border-gray-100 h-fit">
            ...
            <div className="pt-6 border-t border-gray-200 flex flex-col items-center gap-2 text-center">
              ...
              <p className="text-[9px] font-bold text-gray-400 leading-relaxed uppercase tracking-widest">
                Publicly available for all schools
              </p>
            </div>
          </div>
        </div>
