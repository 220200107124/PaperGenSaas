    ...
    const columns = [
        {
            ...
            accessor: (r: SchoolRequest) => (
                <div className="flex items-start sm:items-center gap-3 sm:gap-4 py-2">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gray-50 flex items-center justify-center border border-gray-100">
                        <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400" />
                    </div>
                    <div className="flex flex-col">
                        <span className="font-black text-gray-900 text-sm sm:text-base">{r.schoolName}</span>
                        <div className="flex flex-wrap items-center gap-1 sm:gap-2">
                             <span className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest">{r.city || 'Unknown City'}</span>
                             {r.contactPerson && <span className="text-[9px] sm:text-[10px] font-bold text-brand-blue uppercase tracking-widest border-l border-gray-100 pl-2">{r.contactPerson}</span>}
                        </div>
                    </div>
                </div>
            )
        },
        {
            ...
            accessor: (r: SchoolRequest) => (
                <div className="flex flex-col gap-1">
                    <span className="text-xs sm:text-sm font-bold text-gray-700 break-all">{r.email}</span>
                    <span className="text-[10px] sm:text-[11px] text-gray-400">{r.phone || 'No phone'}</span>
                </div>
            )
        },
        {
        ...
        <div className="space-y-6 sm:space-y-8 lg:space-y-10 px-3 sm:px-4 md:px-6 animate-in fade-in slide-in-from-bottom-6 duration-1000">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 sm:gap-6">
                <div className="space-y-2">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 uppercase italic">Registration Requests</h1>
                    <p className="text-gray-500 font-bold uppercase tracking-[0.2em] text-[9px] sm:text-[10px] flex items-center gap-2">
                        Onboarding <ChevronRight className="w-3 h-3" /> <span className="text-brand-blue">Partner Schools</span>
                    </p>
                </div>
