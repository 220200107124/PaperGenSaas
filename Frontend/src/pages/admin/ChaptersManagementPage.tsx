...
const ChaptersManagementPage: React.FC = () => {
  ...
  const fetchSubjects = async (
    stdId: string,
    setter: React.Dispatch<React.SetStateAction<Subject[]>>,
  ) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (editingChapter) {
          await axios.patch(
            `${import.meta.env.VITE_API_URL}/chapters/${editingChapter.id}`,
            payload,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
      ...
      accessor: (c: Chapter) => (
        <div className="flex items-center sm:items-center gap-3">
          <div className="w-10 h-10 bg-brand-blue/5 rounded-xl flex items-center justify-center text-brand-blue">
            <Layout className="sm:w-5 sm:h-5 w-4 h-4" />
          </div>
          <div>
            <p className="sm:text-sm text-xs font-bold text-gray-900">
              {c.name}
            </p>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
              Chapter UID: {c.id.slice(0, 8)}
            </p>
          </div>
        </div>
      ),
      ...
      accessor: (c: Chapter) => (
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 sm:p-2.5 rounded-xl border border-gray-100 text-gray-400 hover:text-brand-blue hover:bg-brand-blue/5 transition-all shadow-sm"
          >
            <Edit2 className="w-4 h-4" />
          </button>
          <button
            onClick={() => handleDelete(c.id)}
            className="p-2 sm:p-2.5 rounded-xl border border-gray-100 text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all shadow-sm"
          >
    ...
    <div className="space-y-6 sm:space-y-8 lg:space-y-10 px-3 sm:px-4 md:px-6 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-gray-400 font-black uppercase tracking-[0.2em] text-[9px] sm:text-[10px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-white p-4 sm:p-6 md:p-8 rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/20 items-end">
        <div className="space-y-2">
          ...
          {/* <select
            ...
            className="w-full px-4 py-3 sm:px-6 sm:py-4 bg-gray-50 border-0 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-brand-blue/5 transition-all"
          >
        ...
        <div className="space-y-2">
          ...
          <CustomSelect
              ...
              onChange={(val) => formik.setFieldValue("subjectId", val)}
...
//                         <select
//                             // className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-brand-blue/5 focus:bg-white focus:border-brand-blue/20 transition-all"
//                             className="w-full max-w-full px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-brand-blue/5 focus:bg-white focus:border-brand-blue/20 transition-all overflow-hidden text-ellipsis"
...
//                             {standards.map(std => <option key={std.id} value={std.id} title={std.name}>{std.name}</option>)} */}
//                         </select>
...
//                         <select
//                             // className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-brand-blue/5 focus:bg-white focus:border-brand-blue/20 transition-all"
//                             className="w-full max-w-auto px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-bold focus:outline-none focus:ring-4 focus:ring-brand-blue/5 focus:bg-white focus:border-brand-blue/20 transition-all overflow-hidden text-ellipsis"
